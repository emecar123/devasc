...
#create entry for 172.16.1.whatever  hostname -- in this Lab it is called ubuntu
...
#copy the key to other machines (called ubuntu)
ssh-copy-id -i .ssh/id_rsa.pub ubuntu
#note if refused, run:
#sudo apt-get install openssh-server on other machines

#Now configure sudo so that it doesnt require a password
ssh ubuntu1
sudo visudo
#very bottom of file add this line:
knox ALL=(ALL) NOPASSWD: ALL #Knox is the username for the local machine

#then save and exit (ctl+C :qa!)
